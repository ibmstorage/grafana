...
func Initialize(ctx context.Context, cfg *setting.Cfg, opts Options, apiOpts api.ServerOptions) (*Server, error) {
	...
	publicdashboardsStore := publicdashboards.ProvideStore(sqlStore, cfg, featureToggles)
	serviceWrapper := publicdashboards.ProvideServiceWrapper(publicdashboardsStore)
	...
	folderimplService := folderimpl.ProvideService(accessControl, userimplService, featureToggles, bundleregistryService, serviceWrapper, cfg, registerer, tracer, resourceClient, sortService, eventualRestConfigProvider)
	...
	dashboardServiceImpl, err := service7.ProvideDashboardServiceImpl(cfg, sqlStore, featureToggles, folderPermissionsService, accessControl, acimplService, folderimplService, registerer, quotaService, orgService, serviceWrapper, dualwriteService, serverLockService, kvStore, k8sHandlerWithFallback)
	if err != nil {
		return nil, err
	}
	...
	publicdashboardsService := publicdashboards.ProvideService(cfg, featureToggles, publicdashboardsStore, queryServiceImpl, repositoryImpl, accessControl, serviceWrapper, dashboardService, ossLicensingService)
	middleware := publicdashboards.ProvideMiddleware()
	apiApi := publicdashboards.ProvideApi(publicdashboardsService, routeRegisterImpl, accessControl, featureToggles, middleware, cfg, ossLicensingService)
	...
	metricService, err := publicdashboards.ProvideMetricsService(publicdashboardsStore, registerer)
	if err != nil {
		return nil, err
	}
	...
	dashboardsAPIBuilder := dashboard.RegisterAPIService(featureToggles, apiserverService, dashboardService, service13, dashboardServiceImpl, dashboardPermissionsService, accessControl, accessClient, provisioningServiceImpl, registerer, sqlStore, tracingService, resourceClient, dualwriteService, quotaService, eventualRestConfigProvider, userimplService, libraryElementService, publicdashboardsService, serviceImpl, dashboardActivityChannel, configProvider)
	v4 := _wireValue
	decryptAuthorizer := decrypt.ProvideDecryptAuthorizer(tracer, v4)
	...
	v5 := extras.ProvideProvisioningExtraAPIs(webhookExtraBuilder)
	...
	v6 := extras.ProvideExtraWorkers(pullRequestWorker)
	...
	provisioningAPIBuilder, err := provisioning2.RegisterAPIService(cfg, featureToggles, apiserverService, registerer, resourceClient, eventualRestConfigProvider, accessClient, dualwriteService, usageStats, tracingService, v5, v6, repositoryFactory, connectionFactory, quotaGetter)
	if err != nil {
		return nil, err
	}
